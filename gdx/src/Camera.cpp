...
Camera::Camera()
	: direction(0, 0, -1), up(0, 1, 0), nearPlane(1), farPlane(100), viewportWidth(0), viewportHeight(0), m_ray(Vector3(), Vector3())
{
}
...
{
	m_right.set(direction).crs(up).nor();
	up.set(m_right).crs(direction).nor();
}
...
{
	m_tmpMat.setToRotation(m_tmpVec.set(axisX, axisY, axisZ), angle);
	direction.mul(m_tmpMat).nor();
	up.mul(m_tmpMat).nor();
}
...
{
	unproject(m_ray.origin.set(x, y, 0), viewportX, viewportY, viewportWidth, viewportHeight);
	unproject(m_ray.direction.set(x, y, 1), viewportX, viewportY, viewportWidth, viewportHeight);
	m_ray.direction.sub(m_ray.origin).nor();
	return m_ray;
}
